...
async def _describe_opened_flow(
    ...
    credential: Final = (
        None if state != "interactive" else await lookup_vendor_credential(flow.user_id, server.server_id)
    )
    ...
    described: Final[ConnectFlowDescription] = {
        ...
        "connected": state == "m2m" or (credential == "present" if credential is not None else None),
    }
    return described
