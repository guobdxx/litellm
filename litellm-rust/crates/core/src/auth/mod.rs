...
pub type AttemptFuture<'a, T> = Pin<Box<dyn Future<Output = Result<T, Error>> + Send + 'a>>;
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum FollowUpDestinationPolicy {
    Deny,
    SameOrigin,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BodyReplayPolicy {
    Never,
    Replayable,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum AttemptRetryPolicy {
    Never,
    Retryable,
}
...
pub trait AttemptProgressSink: Send + Sync {
...
impl AttemptProgressSink for NoopProgress {
    fn record(&self, _: AttemptProgress) {}
}
...
pub struct AttemptControl {
    ...
    pub cancellation: AttemptCancellation,
    pub progress: Arc<dyn AttemptProgressSink>,
}
...
pub struct AttemptRequest {
    ...
    pub follow_up_destination: FollowUpDestinationPolicy,
    pub retry: AttemptRetryPolicy,
    pub body_replay: BodyReplayPolicy,
}
...
pub async fn send_attempt<Callback, CallbackFuture>(
    client: &Client,
    origin: &Url,
...
where
    Callback: FnOnce(AttemptView) -> CallbackFuture,
    CallbackFuture: Future<Output = Result<AttemptDecision, Error>>,
{
    validate_destination(origin, &request)?;
    control.progress.record(AttemptProgress::Validated);

    control
        .progress
        .record(AttemptProgress::CredentialPreparationStarted);
    let preparation = control.run(authorization.prepare()).await?;
    let mut headers = request.headers;
    apply_preparation(&mut headers, &preparation)?;
    control.progress.record(AttemptProgress::CallbackStarted);
    let decision = control
        .run(callback(AttemptView {
        ...
        .await?;
    control.progress.record(AttemptProgress::CallbackComplete);
    let body = resolve_body(request.body, decision)?;
    let body = match body {
        AttemptBody::Bodyless => None,
        AttemptBody::JsonObject(value) => Some(serde_json::to_vec(&value).map_err(|error| {
            Error::InvalidRequest(format!("failed to encode request: {error}"))
        })?),
    };
    control.progress.record(AttemptProgress::Encoded);
    ...
    control
        .progress
        .record(AttemptProgress::AuthorizationStarted);
    let authorized = authorize(validated, preparation.authorizer.as_ref(), control).await?;
    control.progress.record(AttemptProgress::Authorized);
    control
        .progress
        .record(AttemptProgress::TransmissionStarted);
    let response = control
        .run(async { client.execute(authorized.0).await.map_err(transport_error) })
        .await?;
    control.progress.record(AttemptProgress::ResponseStarted);
    ...
    let body = control
        ...
        .await?;
    control.progress.record(AttemptProgress::ResponseBodyRead);
    Ok(AttemptResponse {
        status,
        headers,
        body,
...
fn validate_destination(origin: &Url, request: &AttemptRequest) -> Result<(), Error> {
    if Origin::parse(origin)? != Origin::parse(&request.url)? {
        return Err(destination_error());
    }
    if request.operation == AttemptOperation::FollowUp
        && request.follow_up_destination == FollowUpDestinationPolicy::Deny
    {
        return Err(destination_error());
    }
    Ok(())
...
fn resolve_body(body: AttemptBody, decision: AttemptDecision) -> Result<AttemptBody, Error> {
    match decision {
        AttemptDecision::Unchanged => Ok(body),
        AttemptDecision::Reject(message) => Err(Error::InvalidRequest(message)),
        AttemptDecision::Replace(value) => match body {
            AttemptBody::Bodyless => Err(Error::InvalidRequest(
                "callback cannot add a body to a bodyless operation".into(),
            )),
            AttemptBody::JsonObject(_) => value
                .as_object()
                .cloned()
                .map(AttemptBody::JsonObject)
                .ok_or_else(|| {
