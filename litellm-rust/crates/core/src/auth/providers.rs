...
use super::{
    AttemptFuture, AuthorizationInput, AuthorizationMutation, AuthorizationPreparation,
    AuthorizationProvider, Clock, CredentialProvenance, RequestAuthorizer, SecretString,
...
type CredentialFactory =
    Box<dyn FnOnce() -> AttemptFuture<'static, CredentialSpec> + Send + 'static>;
...
impl CredentialCandidate {
    pub fn new(
        provenance: CredentialProvenance,
        kind: CredentialKind,
        factory: impl FnOnce() -> AttemptFuture<'static, CredentialSpec> + Send + 'static,
    ) -> Self {
        Self {
            provenance,
            kind,
...
impl CredentialAdapterRegistry {
    ...
    ) -> AttemptFuture<'_, Arc<dyn AuthorizationProvider>> {
        let selected = candidates.into_iter().next();
        Box::pin(async move {
            let candidate = selected.ok_or_else(|| Error::Auth("missing credential".into()))?;
            let expected_kind = candidate.kind;
            let provenance = candidate.provenance;
            let spec = (candidate.factory)().await?;
            ...
            adapter.build(spec, provenance, self.clock.clone())
        })
    }
}
