...
use litellm_core::auth::{AttemptFuture, SecretString, TokenCredential, TokenProvider};
...
"#,
    ...
    #[test]
    fn async_providers_preserve_context_lifetime_concurrency_and_attempt_count() {
        Python::initialize();
        Python::attach(|py| {
            let module = test_module(py);
            let locals = PyDict::new(py);
